...
QVariant GameBoard::data(const QModelIndex &index, int role) const
{
    ...
    const int rawIndex = {index.row()};

    if(!isPositionValid(rawIndex))
        return {};

    return QVariant::fromValue(m_rawBoard[rawIndex].value);
}
...
bool GameBoard::isBoardValid() const
{
    ...
    const size_t start_poitn = 1;

    for(size_t i = 0; i < m_boardSize; ++i){
        if(m_rawBoard[i].value == m_boardSize){
            inv += start_poitn + i / m_dimension;
        }
    }
